	...
	describe('onPlayerReady', () => {
		...
		it('Should set Off option as selected when there is no default track', () => {
			vgTrackSelector.API.getMasterMedia = () => {
				return {
					elem: {
						children: [createSubtitleTrack('English', 'en', false), createSubtitleTrack('Español', 'es', false)]
					}
				} as any
			}
			vgTrackSelector.onPlayerReady()
	...
	describe('selectTrack', () => {
		beforeEach(() => {
			spyOn(vgTrackSelector.API, 'getMasterMedia').and.returnValue({
				elem: {
					textTracks: [{ mode: 'showing', language: 'en' }, { mode: 'hidden', language: 'es' }]
				}
			})
		})
		it('Should select by track id', () => {
			...
			expect(
				(vgTrackSelector.API.getMasterMedia().elem as any).textTracks.filter(item => item.mode === 'showing')[0]
					.language
			).toBe('es')
		})
		it('Should select Off when track id is null', () => {
			...
			expect(
				(vgTrackSelector.API.getMasterMedia().elem as any).textTracks.filter(item => item.mode === 'showing').length
			).toBe(0)
		})
	})
})
