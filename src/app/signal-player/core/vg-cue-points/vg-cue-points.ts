...
export class VgCuePoints implements OnInit, OnDestroy {
	...
	ngOnInit() {
		let onLoad = Observable.fromEvent(this.ref.nativeElement, SignalEvents.VG_LOAD)
	...
	updateCuePoints(cues: TextTrackCue[]) {
		...
		for (let i = 0, l = cues.length; i < l; i++) {
			let onEnter = Observable.fromEvent(cues[i], SignalEvents.VG_ENTER)
			this.cuesSubscriptions.push(onEnter.subscribe(this.onEnter.bind(this)))

			let onExit = Observable.fromEvent(cues[i], SignalEvents.VG_EXIT)
